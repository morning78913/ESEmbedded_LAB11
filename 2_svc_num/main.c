...
void svc_handler_c(uint32_t lr_value, uint32_t msp_value) //void svc_handler_c(uint32_t lr_value, ??????)
{
	printf("[SVC Handler] LR: 0x%X\r\n", (unsigned int)lr_value);
	printf("[SVC Handler] MSP Backup: 0x%X \r\n", (unsigned int)msp_value); //printf("[SVC Handler] MSP Backup: 0x%X \r\n", (unsigned int)??????);
	...
	if ((lr_value & 0b0100)>>2) //if (??????) //Test bit 2 of EXC_RETURN
	{
		stack_frame_ptr = (uint32_t *)read_psp(); //stack_frame_ptr = ?????? //if 1, stacking used PSP
		printf("[SVC Handler] Stacking used PSP: 0x%X \r\n\n", (unsigned int)stack_frame_ptr);
	}
	else
	{
		stack_frame_ptr = (uint32_t *)msp_value; //stack_frame_ptr = ?????? //if 0, stacking used MSP
		printf("[SVC Handler] Stacking used MSP: 0x%X \r\n\n", (unsigned int)stack_frame_ptr);
	}

	uint32_t stacked_r0 = *(stack_frame_ptr); //uint32_t stacked_r0 = ??????
	uint32_t stacked_r1 = *(stack_frame_ptr + 1); //uint32_t stacked_r1 = ??????
	uint32_t stacked_return_addr = *(stack_frame_ptr + 6); //uint32_t stacked_return_addr = ??????

	uint16_t svc_instruction = *((uint16_t *)stacked_return_addr - 1); //uint16_t svc_instruction = ??????
	uint8_t svc_num = (uint8_t)svc_instruction; //uint8_t svc_num = ??????
